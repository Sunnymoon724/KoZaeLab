...
namespace KZLib
{
	public partial class SoundMgr : LoadSingletonMB<SoundMgr>
	{
		...
		public AudioSource SetBGM(string audioPath)
		{
			...
			{
				KZLogType.System.E("Audio path is empty");

				return null;
			}

			return SetBGM(ResMgr.In.GetAudioClip(audioPath));
		}
		...
		public AudioSource SetBGM(AudioClip audioClip)
		{
			if(!audioClip)
			{
				KZLogType.System.E("Audio clip is null");
		...
		public AudioSource PlayBGM(string audioPath,float time = 0.0f,float delay = 0.0f)
		{
			...
			return PlayBGM(ResMgr.In.GetAudioClip(audioPath),time,delay);
		}
