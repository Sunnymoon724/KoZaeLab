...
/// <summary>
/// Filled <see cref="Image"/> driven by a clamped value range. Supports optional gradient tint and animated value tweening.
/// </summary>
public class ProgressImage : BaseImage
{
	...
	public float CurrentValue
	{
		...
		private set
		{
			...
			if(m_useGradient && m_image && m_gradientColor != null)
			{
	...
	public float CurrentProgress
	{
		get
		{
			var range = m_maxValue-m_minValue;

			return range <= 0.0f ? 0.0f : (CurrentValue-m_minValue)/range;
		}
	}
	...
	{
		var targetValue = Mathf.Clamp(value,m_minValue,m_maxValue);
		...
		await KZExternalKit.ExecuteProgressAsync(CurrentValue,targetValue,duration,_SetValue,ignoreTimescale,animationCurve,token);
	}
