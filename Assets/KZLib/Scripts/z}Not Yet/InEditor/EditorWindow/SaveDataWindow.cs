	...
	public class SaveDataWindow : OdinEditorWindow
	{
		...
		private class SaveData
		{
			...
			public string Data
			{
				...
				private set
				{
					...
					var data = m_IsEncrypted ? CommonUtility.EncryptAES(value,CommonUtility.GenerateAESKeyByPassword(m_TableName)) : value;
			...
			public string RealKey => m_IsEncrypted ? CommonUtility.EncryptAES(m_Key,CommonUtility.GenerateAESKeyByPassword(m_TableName)) : m_Key;
		...
		private string TableName
		{
			...
			set
			{
				...
				try
				{
					var key = CommonUtility.EncryptAES(data.Key,CommonUtility.GenerateAESKeyByPassword(value));
				}
				...
				{
					var key = isEncrypted ? CommonUtility.DecryptAES(pair.Key,CommonUtility.GenerateAESKeyByPassword(TableName)) : pair.Key;
					var result = isEncrypted ? CommonUtility.DecryptAES(pair.Value,CommonUtility.GenerateAESKeyByPassword(TableName)) : pair.Value;
