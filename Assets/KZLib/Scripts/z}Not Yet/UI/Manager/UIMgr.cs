...
namespace KZLib
{
	public partial class UIMgr : LoadSingletonMB<UIMgr>
	{
		...
		public TTag Find<TTag>(UITag _type) where TTag : class,IWindowUI
		{
			var window = FindOpened(_type);

			return (window != null) ? window as TTag : null;
		}

		private WindowUI FindOpened(UITag _tag)
		{
			foreach(var repository in m_RepositoryList)
			{
				var window = repository.FindOpenedUI(_tag);
