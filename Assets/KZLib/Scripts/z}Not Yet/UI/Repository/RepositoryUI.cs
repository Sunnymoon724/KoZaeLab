...
public abstract class RepositoryUI : SortingLayerCanvas
{
	...
	public void Add(WindowUI _window)
	{
		...
		_window.SetParentSortingLayer(this,FindUIOrder(_window.Tag,_window.Priority));
	...
	public void Remove(WindowUI _window,bool _release)
	{
		...
		if(_release)
		{
			...
			{
				_window.gameObject.DestroyObject();
			}
		}
	}

	private int FindUIOrder(UITag _tag,UIPriorityType _priority)
	{
		...
		return dataList.Max(x=>x.LocalSortingLayerOrder)+c_sorting_interval;
	}

	public WindowUI FindOpenedUI(UITag _tag)
	{
