...
namespace KZLib
{
	public class SaveDataHandler
	{
		...
		{
			var data = m_Encrypt ? CommonUtility.EncryptAES(_data,CommonUtility.GenerateAESKeyByPassword(m_TableName)) : _data;
		...
		private string EncryptText(string _text)
		{
			...
			return m_Encrypt ? CommonUtility.EncryptAES(_text,CommonUtility.GenerateAESKeyByPassword(m_TableName)) : _text;
		}

		private string DecryptText(string _text)
		{
			return m_Encrypt ? CommonUtility.DecryptAES(_text,CommonUtility.GenerateAESKeyByPassword(m_TableName)) : _text;
		}
	}
}