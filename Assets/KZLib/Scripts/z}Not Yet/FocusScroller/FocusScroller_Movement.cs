...
namespace KZLib.Develop
{
	public partial class FocusScroller : BaseComponentUI,IPointerUpHandler,IPointerDownHandler,IBeginDragHandler,IEndDragHandler,IDragHandler,IScrollHandler
	{
		...
		private class ScrollData
		{
			...
			{
				_SetData(true,true,0.0f,EaseType.OutCubic,0.0f,0.0f,_onComplete);
			}

			public void SetData(float _duration,EaseType _type,float _endLocation,Action _onComplete)
			{
				_SetData(true,false,_duration,_type,Time.unscaledTime,_endLocation,_onComplete);
			}
			...
			private void _SetData(bool _enable = false,bool _elastic = false,float _duration = 0.0f,EaseType _type = EaseType.OutCubic,float _startTime = 0.0f,float _endLocation = 0.0f,Action _onComplete = null)
			{
				Enable = _enable;
				Elastic = _elastic;
				Duration = _duration;
		...
		public void ScrollTo(float _location,float _duration,EaseType _type)
		{
			...
			var endLocation = m_CurrentLocation+CalculateLoopedMovementAmount(m_CurrentLocation,_location);
