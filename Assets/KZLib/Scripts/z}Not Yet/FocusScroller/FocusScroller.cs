...
namespace KZLib.Develop
{
	public partial class FocusScroller : BaseComponentUI,IPointerUpHandler,IPointerDownHandler,IBeginDragHandler,IEndDragHandler,IDragHandler,IScrollHandler
	{
		...
		public void UpdateIndex(int _index)
		{
			...
			if(m_PrevButton)
			{
				m_PrevButton.gameObject.SetActiveIfDifferent(m_CircularMode || m_FocusIndex != 0);
			}

			if(m_NextButton)
			{
				m_NextButton.gameObject.SetActiveIfDifferent(m_CircularMode || m_FocusIndex != m_CellList.Count-1);
			}
		}
		...
		private void ResizePool(float _firstLocation)
		{
			...
			{
				m_SlotList.Add(m_ObjectPool.GetOrCreate(m_Viewport));
			}
		}
