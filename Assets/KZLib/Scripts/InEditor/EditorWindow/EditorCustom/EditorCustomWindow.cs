...
namespace KZLib.KZWindow
{
	public class EditorCustomWindow : OdinEditorWindow
	{
		...
		// protected bool UsePrefixDesign { get => EditorCustom.GetCustomInfo<bool>(nameof(UsePrefixDesign)) && UseHierarchy; set => EditorCustom.SetCustomInfo(nameof(UsePrefixDesign),value); }
		// [HorizontalGroup("Hierarchy/Data/1"),HideLabel,KZHexColor,ShowInInspector,ShowIf(nameof(UsePrefixDesign))]
		// protected List<PrefixDesignInfo> DesignInfoList { get => EditorCustom.GetCustomInfo<string>(nameof(CategoryHexColor)); set => EditorCustom.SetCustomInfo(nameof(CategoryHexColor),value); }
