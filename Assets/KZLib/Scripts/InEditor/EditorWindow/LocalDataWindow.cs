	...
	public class LocalDataWindow : OdinEditorWindow
	{
		...
		private class LocalData
		{
			...
			public string Value
			{
				...
				private set
				{
					...
					LocalDataMgr.In.SetObject(m_tableName,CommonUtility.EncryptAES(m_key,m_password),CommonUtility.EncryptAES(value,m_password));
		...
		private string TableName
		{
			...
			set
			{
				...
				var dataDict = LocalDataMgr.In.GetDataDictInTable(value);
		...
		private List<string> TableNameList
		{
			get
			{
				...
				{
					if(LocalDataMgr.HasInstance)
					{
						LocalDataMgr.In.Dispose();
					}

					m_tableNameList = new List<string>(LocalDataMgr.In.GetTableNameGroup());
				}
		...
		private void OnRemoveData(int index)
		{
			...
			LocalDataMgr.In.RemoveKey(TableName,key);
		...
		{
			LocalDataMgr.In.DeleteTable(TableName);
