...
namespace KZLib.KZMenu
{
	public static partial class AssetsMenuItem
	{
		...
		{
			if(CommonUtility.DisplayCancelableProgressBar("에셋 파인더 로드","에셋 파인더 로드 중",0.1f))
			{
			...
			foreach(var path in pathGroup)
			{
				...
				if(CommonUtility.DisplayCancelableProgressBar("에셋 파인더 로드",string.Format("에셋 파인더 로드 중 [{0}/{1}]",index,totalCount),index++,totalCount))
				{
					CommonUtility.ClearProgressBar();
