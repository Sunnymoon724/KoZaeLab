...
public class VideoPanel : BasePanel
{
	...
	public async UniTask PrepareVideoAsync(VideoInfo videoInfo)
	{
		...
		{
			var skipPanel = UIManager.In.Open(CommonUINameTag.SkipPanel,new SkipPanel.Param(Stop)) as SkipPanel;
	...
	public async UniTask WaitForPreparedAsync()
	{
		...
		await UniTask.WaitUntil(_IsPrepared).SuppressCancellationThrow();
	}
