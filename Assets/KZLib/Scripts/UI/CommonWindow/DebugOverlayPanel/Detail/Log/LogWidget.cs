...
namespace KZLib.KZWidget.Debug
{
	public class LogWidget : BaseComponent
	{
		...
		private LogEntryInfo _CreateLogEntryInfo(MessageInfo messageInfo)
		{
			...
			if(headerInfo != null)
			{
				var logInfo = m_logInfoDict[headerInfo.LogType];
		...
		private HeaderInfo _SplitHeader(string header)
		{
			...
			{
				var type = headerArray[0].TrimAngleBrackets().ToEnum<LogInfoType>();
		...
		private record HeaderInfo(LogInfoType LogType,string Time);
	}
}