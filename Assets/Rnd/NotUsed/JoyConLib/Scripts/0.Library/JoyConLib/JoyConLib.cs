...
public class JoyConLib
{
	...
	public JoyConLib(IntPtr handle_, bool imu, bool localize, float alpha, bool left)
    {
		handle = handle_;
		imu_enabled = imu;
    ...
    public bool GetButtonDown(out Button _button)
    {
    ...
    public bool GetButton(out Button _button)
    {
    ...
    public bool GetButtonUp(out Button _button)
    {
    ...
    Button CheckRL(int _index)
    {
        switch((Button)_index)
        {
            case Button.DPAD_DOWN:
                return isLeft ? Button.DPAD_DOWN : Button.DPAD_UP;
            case Button.DPAD_UP:
                return isLeft ? Button.DPAD_UP : Button.DPAD_DOWN;
            case Button.DPAD_RIGHT:
                return isLeft ? Button.DPAD_RIGHT : Button.DPAD_LEFT;           
            case Button.DPAD_LEFT:
                return isLeft ? Button.DPAD_LEFT : Button.DPAD_RIGHT;
            default:
    ...
    {
        return isLeft ? new Vector2(stick[1],stick[0]) : new Vector2(-stick[1],-stick[0]);
    }
    ...
    public void Update()
    {
        if (state > state_.NO_JOYCONS)
        {
            ...
            while (reports.Count > 0)
            {
                ...
                ts_de = report_buf[1];
                //DebugPrint($"Dequeue. Queue length: {reports.Count:d}. Packet ID: {report_buf[0]:X2}. Timestamp: {report_buf[1]:X2}. Lag to dequeue: {System.DateTime.Now.Subtract(rep.GetTime()):s}. Lag between packets (expect 15ms): {rep.GetTime().Subtract(ts_prev):s}", DebugType.THREADING);
                ts_prev = rep.GetTime();
            }
    ...
    {
        if (state <= JoyConLib.state_.ATTACHED) return;
